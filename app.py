...
def parse_items(text):
    lines = text.split('\n')
    cleaned = []
    skip_keywords = ["balance to pay", "visa", "total", "payment", "thank", "store", "auth", "purchase", "items in transaction", "bag fee", "tax", "deposit"]

    # Merge standalone price lines into previous line
    i = 0
    while i < len(lines):
        line = lines[i].strip()
        if re.match(r'^\$?\d+\.\d{2}$', line) and i > 0:
            lines[i-1] += ' ' + line
        else:
            cleaned.append(line)
        i += 1
